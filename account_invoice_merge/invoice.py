...
class account_invoice(osv.osv):
    ...
    # compute what the new orders should contain
        ...
        for porder in [order for order in self.browse(cr, uid, ids, context=context) if order.state == 'draft']:
            ...
            invoice_lines[porder.id] = []
            for order_line in porder.invoice_line:
                invoice_lines[porder.id].append(invoice_line_pool.copy_data(cr, uid, order_line.id, default={'invoice_id': False}))
                #o_line = order_infos['invoice_line']
            ...
            order_infos['invoice_line'] = invoice_lines
        ...
        for order_key, (order_data, old_ids) in new_orders.iteritems():
            ...
            for old_id in old_ids:
                new_invoice_lines += invoice_lines[old_id]
            order_data['invoice_line'] = [(0, 0, value) for value in new_invoice_lines]
